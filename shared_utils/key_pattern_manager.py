...
def is_ark_api_key(key):
    # ark_api_key = '5f017bfa-83b0-46ee-ac8d-5909b8fb41af'
    API_MATCH_ARK = re.match(r"[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{12}$", key)
    return bool(API_MATCH_ARK)
...
def is_any_api_key(key):
    if ',' in key:
        ...
        return is_openai_api_key(key) or is_api2d_key(key) or is_azure_api_key(key) or is_cohere_api_key(key) or is_ark_api_key(key)
...
def select_api_key(keys, llm_model):
    ...
    if llm_model.startswith('ep-'):
        for k in key_list:
            if is_ark_api_key(k): avail_key_list.append(k)
